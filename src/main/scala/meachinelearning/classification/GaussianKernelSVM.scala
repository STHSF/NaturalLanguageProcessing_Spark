...
object GaussianKernelSVM {
  ...
  def main(arg: Array[String]) {
    ...
    val data = DenseMatrix(Source.fromFile("/Users/li/Kunyan/NaturalLanguageProcessing/src/main/scala/meachinelearning/data/SupportVectorMachineWithGaussianKernel.txt")
      .getLines().toList.flatMap(_ match {
      case reg(x1, x2, label) => Seq((x1.toDouble, x2.toDouble, label.toDouble))
      case _ => Seq.empty
    }): _*)
  ...
  def RBFKernel(sigma: Double)(x1: DenseVector[Double], x2: DenseVector[Double]): Unit = {

  }

  // sigmoid kernel
  def sigmoidKernel(sigma: Double)(x1: DenseVector[Double], x2: DenseVector[Double]): Unit = {
  ...
  def predict(model: Model)(X: DenseMatrix[Double]): DenseVector[Double] = {
    ...
    for (i <- 0 until X.rows) {
      ...
      predictRes(i) = I(prediction + model.b >= 0)
    }
        ...
        if ((Y2(i) * E(i) < -tol && alphas(i) < C) || (Y2(i) * E(i) > tol && alphas(i) > 0)) {
          ...
          if (L != H && eta < 0) {
            ...
            if (math.abs(alphas(j) - alpha_j_old) < tol) {
              ...
              val b1 = b - E(i) - (Y2(i) * (alphas(i) - alpha_i_old) * K(i, j))
              - (Y2(j) * (alphas(j) - alpha_j_old) * K(i, j))
              val b2 = b - E(j) - (Y2(i) * (alphas(i) - alpha_i_old) * K(i, j))
  ...
  def plotDecisionBoundary(plot: Plot, X: DenseMatrix[Double], y: DenseVector[Double], model: Model) = {
    ...
    val NUM = 100
    val x1 = linspace(min(X(::, 0)), max(X(::, 0)), NUM)
    val x2 = linspace(min(X(::, 1)), max(X(::, 1)), NUM)
